...
// GET /coach/attendance/:enrollmentId/report
router.get('/attendance/:enrollmentId/report', auth, async (req, res) => {
  try {
    if (req.user.role !== 'coach') {
      return res.status(403).json({ message: 'Access denied: Coach role required' });
    }
    ...
    const { enrollmentId } = req.params;
    console.log('Generating attendance report for enrollment:', enrollmentId);
    ...
    .populate('program', 'name sport institute');

    if (!enrollment) {
      return res.status(404).json({ message: 'Enrollment not found or not assigned to this coach' });
    }
    ...
    const report = {
      student: {
        ...
        institute: enrollment.program.institute
      },
      summary: {
        totalDays,
        presentDays,
...
router.get('/program/:programId/students', auth, async (req, res) => {
  try {
    if (req.user.role !== 'coach') {
      return res.status(403).json({ message: 'Access denied: Coach role required' });
    }
    ...
    // Verify that the program is assigned to the coach
    const isProgramAssigned = coach.assignedPrograms.some(p => p._id.toString() === req.params.programId);
    if (!isProgramAssigned) {
      return res.status(403).json({ message: 'You are not assigned to this program' });
    }

    const enrollments = await Enrollment.find({ program: req.params.programId })
      .populate('parent', 'name email')
      .select('childName parent program');
...
router.post('/attendance/bulk', auth, async (req, res) => {
  try {
    ...
    const attendanceRecords = await Promise.all(attendanceData.map(async (record) => {
      ...
      if (!enrollment) {
        throw new Error(`Enrollment not found for ID: ${record.enrollmentId}`);
      }

      // Verify that the program is assigned to the coach
      const isProgramAssigned = coach.assignedPrograms.some(p => 
        p._id.toString() === enrollment.program.toString()
      );
      if (!isProgramAssigned) {
        throw new Error(`You are not assigned to the program for enrollment: ${record.enrollmentId}`);
      }
