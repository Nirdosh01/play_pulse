...
router.get('/payment/success', async (req, res) => {
  try {
    const { purchase_order_id, status, token, amount, pidx } = req.query;
    console.log('Payment success query params:', req.query);

    if (!purchase_order_id) {
      console.log('No purchase_order_id provided');
    ...
    // Construct query string with all relevant parameters
    ...
    res.redirect(`http://localhost:3000/parent?${queryParams}`);
  } catch (err) {
    console.error('Payment success error:', err.message);
    res.redirect(`http://localhost:3000/parent?payment=error&tab=enrollments`);
  }
});

// Get attendance report
router.get('/attendance/:studentId/report', auth, async (req, res) => {
  console.log('Entering GET /attendance/:studentId/report');
  try {
    const enrollment = await Enrollment.findById(req.params.studentId)
      .populate('program', 'name')
      .populate('institute', 'name');
    
    if (!enrollment || enrollment.parent.toString() !== req.user.id) {
      console.log('Unauthorized or enrollment not found:', req.params.studentId);
      return res.status(403).json({ message: 'Unauthorized' });
    }
    
    const attendance = await Attendance.find({ enrollment: req.params.studentId }).sort({ date: 1 });
    ...
    const report = {
      student: {
        ...
        institute: enrollment.institute.name
      },
      attendance: attendance.map(a => ({
        date: a.date,
        status: a.status,
        session: a.session || 'N/A'  // Include session information
      })),
      summary: {
        totalDays,
        presentDays,
    ...
    console.error('Error in GET /attendance/:studentId/report:', err.message, err.stack);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
