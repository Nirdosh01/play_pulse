...
router.post('/events', auth, upload.array('images', 5), async (req, res) => {
  try {
    ...
    const institute = await Institute.findOne({ owner: req.user.id });
    if (!institute) {
      console.log('Institute not found for owner:', req.user.id);
      return res.status(404).json({ message: 'Institute not found' });
    }
    ...
    const requiredFields = ['name', 'place', 'type', 'date'];
    ...
    if (err.code === 11000) {
      return res.status(400).json({ 
        message: 'Duplicate event found. Please check the event details.' 
      });
    }
    res.status(500).json({ 
      message: 'Server error in event creation', 
      error: err.message,
  ...
  try {
    ...
    if (req.body.existingImages) {
      try {
        ...
        updatedImages = existingEvent.images || [];
      }
    }

    // Add new uploaded images
    if (files && files.length > 0) {
      updatedImages = [...updatedImages, ...files.map(file => `/uploads/${file.filename}`)];
    }
